...
class FilterDataTableDeviceCommandsAction
{
    ...
    {
        $query = DeviceCommand::deviceId($deviceId)->with('deviceCommandType:id,name');

        $relations = ['deviceCommandType'];
                    ...
                    if (in_array($relation, $relations)) {
                        $query->orderBy(function (QueryBuilder $query) use ($relation, $column) {
                            ...
                            $query->select(Str::snake($column))
                                ...
                                ->whereColumn($modelQualifiedName::getTableName() . '.id', DeviceCommand::getTableName() . '.' . Str::snake($relation) . '_id')
                                ->limit(1);
        ...
        if (isset($data['filterModel'])) {
                ...
                foreach ($filterOptions->items as $filterItem) {
                    if (isset($filterItem->value) && $filterItem->value !== '') {
                        if (Str::contains($filterItem->columnField, '.')) {
                            ...
                            $query->where(DeviceCommand::getTableName() . '.' . Str::snake($filterItem->columnField), 'ILIKE', "%{$filterItem->value}%");
                        }
                    }
                }
            }
            ...
            if (isset($filterOptions->quickFilterValues)) {
                $quickFilterColumns = ['id', 'payload', 'deviceCommandType:name'];
                        ...
                        $query->where(function (EloquentBuilder $query) use ($quickFilterColumns, $quickFilterValue) {
                            foreach ($quickFilterColumns as $quickFilterColumn) {
                                if (Str::contains($quickFilterColumn, ':')) {
                                    ...
                                    $query->orWhere->where(DeviceCommand::getTableName() . '.' . $quickFilterColumn, 'ILIKE', "%{$quickFilterValue}%");
                                }
                            }
                        });
                    }
        ...
        return new DeviceCommandCollectionPagination($query->paginate($pageSize));
    }
}
